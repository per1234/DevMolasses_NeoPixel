...
DevMolasses_NeoPixel::DevMolasses_NeoPixel(uint16_t n, uint8_t p, uint8_t t) :
...
DevMolasses_NeoPixel::~DevMolasses_NeoPixel() {
  if(pixels) free(pixels);
  pinMode(pin, INPUT);
}

void DevMolasses_NeoPixel::begin(void) {
  pinMode(pin, OUTPUT);
  digitalWrite(pin, LOW);
}

void DevMolasses_NeoPixel::show(void) {
...
void DevMolasses_NeoPixel::setPin(uint8_t p) {
  pinMode(pin, INPUT);
  pin = p;
  pinMode(p, OUTPUT);
  digitalWrite(p, LOW);
...
void DevMolasses_NeoPixel::setPixelColor(
...
void DevMolasses_NeoPixel::setPixelColor(uint16_t n, uint32_t c) {
  if(n < numLEDs) {
    uint8_t
      r = (uint8_t)(c >> 16),
      g = (uint8_t)(c >>  8),
...
uint32_t DevMolasses_NeoPixel::Color(uint8_t r, uint8_t g, uint8_t b) {
  return ((uint32_t)r << 16) | ((uint32_t)g <<  8) | b;
}

// Query color from previously-set pixel (returns packed 32-bit RGB value)
uint32_t DevMolasses_NeoPixel::getPixelColor(uint16_t n) const {
  if(n >= numLEDs) {
...
uint8_t *DevMolasses_NeoPixel::getPixels(void) const {
  return pixels;
}

uint16_t DevMolasses_NeoPixel::numPixels(void) const {
  return numLEDs;
}
...
void DevMolasses_NeoPixel::setBrightness(uint8_t b) {
...
uint8_t DevMolasses_NeoPixel::getBrightness(void) const {
  return brightness - 1;
}
